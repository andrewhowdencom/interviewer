...
func TestGeminiQuestionProvider_NextQuestion(t *testing.T) {
	t.Run("should return questions until the max number of questions is reached", func(t *testing.T) {
		...
		// Mock the first question
		firstQuestionResp := &genai.GenerateContentResponse{
			Candidates: []*genai.Candidate{
				{
					Content: &genai.Content{
						Parts: []genai.Part{
							genai.Text("This is the first question."),
						},
					},
				},
			},
		}
		mockChat.On("SendMessage", mock.Anything, mock.Anything).Return(firstQuestionResp, nil).Once()

		// Test the first question
		question, hasMore := provider.NextQuestion("")
		assert.True(t, hasMore)
		assert.Equal(t, "This is the first question.", question)
	...
	t.Run("should terminate the interview when INTERVIEW_COMPLETE is received", func(t *testing.T) {
		// Create a mock ChatSession
		mockChat := new(MockChatSession)
